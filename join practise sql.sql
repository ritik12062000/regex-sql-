...
select e.emp_name, d.dep_name from employees as e
left join departments as d
on e_dep_id =dep.dept_id;
#Display all employees and their department names, including employees without departments.
#| emp_name | dept_name |
...
#Display all departments and the total salary paid in each department.
# dept_name , total_salary ?
select  d.dept_name , count(e.emp_id) from empolyees as e
inner join  departments as d
on e.dept_id=dept_id
group by  d.dept_name;


select  m.emp_name , count(*) as total_size 
from employees as e  join employees as m 
on e.manager_id=m.emp_id group by m.emp_id
